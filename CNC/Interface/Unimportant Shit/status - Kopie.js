  ...
  var code = data.map((val, index) => {
    ...
    if(val.workload === 0) {
      wlButton = '<label class="switch"> ' +
        '<input name="' + val.id + '" type="checkbox" onchange = "POSTRequestStatus(this)"> ' +
        '<div class="slider round"></div> ' +
        '</label>';
    } else {
      wlButton = '<label class="switch"> ' +
        '<input name="' + val.id + '" type="checkbox" onchange = "POSTRequestStatus(this)" checked> ' +
        '<div class="slider round"></div> ' +
        '</label>';
    }
